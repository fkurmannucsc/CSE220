...
def get_icache(descriptor_data, sim_path, output_dir):
  ...
  try:
    for config_key in descriptor_data["configurations"].keys():
      ...
      for benchmark in benchmarks_org:
        ...
        avg_icache_config += ratio

        cnt_benchmarks = cnt_benchmarks + 1
        if len(benchmarks_org) > len(benchmarks):
          benchmarks.append(benchmark_name)

        icache_config.append(ratio)

      num = len(benchmarks)
      print(benchmarks)
      icache_config.append(avg_icache_config/num)
      icache[config_key] = icache_config

    benchmarks.append('Avg')
    plot_data(benchmarks, icache, 'Icache Miss Ratio', output_dir+'/FigureC.png')
...
if __name__ == "__main__":
    ...
    get_icache(descriptor_data, args.simulation_path, args.output_dir)
    plt.grid('x')
    plt.tight_layout()
    plt.show()
