...
def get_misprediction(descriptor_data, sim_path, output_dir):
  ...
  try:
    for config_key in descriptor_data["configurations"].keys():
      ...
      for benchmark in benchmarks_org:
        ...
        avg_misprediction_config += ratio

        cnt_benchmarks = cnt_benchmarks + 1
        if len(benchmarks_org) > len(benchmarks):
          benchmarks.append(benchmark_name)

        misprediction_config.append(ratio)

      num = len(benchmarks)
      print(benchmarks)
      misprediction_config.append(avg_misprediction_config/num)
      misprediction[config_key] = misprediction_config

    benchmarks.append('Avg')
    plot_data(benchmarks, misprediction, 'Branch Misprediction Ratio', output_dir+'/FigureB.png')
...
if __name__ == "__main__":
    ...
    get_misprediction(descriptor_data, args.simulation_path, args.output_dir)
    plt.grid('x')
    plt.tight_layout()
    plt.show()
